...
def generate_factor(start_date, end_date):
    ...
    sql = """
    select trade_date, stock_code, trade_time, high, low from tmindata
    ...
    def f(df):
        h = df.set_index(['trade_time', 'stock_code']).loc[:, 'high'].unstack()
        l = df.set_index(['trade_time', 'stock_code']).loc[:, 'low'].unstack()
        h = h.loc[h.index>='093000']
        l = l.loc[l.index>='093000']
        hl = (np.log(h) - np.log(l)).mean()
        return hl
    ...
    df_new.to_sql('tfactorminhl', engine, schema='factor', if_exists='append', index=True, chunksize=10000, method=tools.mysql_replace_into)
