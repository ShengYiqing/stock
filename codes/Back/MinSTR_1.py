...
def generate_factor(start_date, end_date):
    ...
    try:
        sql = """
        CREATE TABLE `factor`.`tfactorminstr` (
    ...
    for trade_date in trade_cal:
        ...
        sql = """
        select STOCK_CODE, TRADE_TIME, VOL from tmindata{TRADE_DATE}
        where trade_time >= '092500'
        and trade_time <= '150000'
        """.format(TRADE_DATE=trade_date)
        df_sql = pd.read_sql(sql, engine)
        VOL = df_sql.set_index(['TRADE_TIME', 'STOCK_CODE']).loc[:, 'VOL']
        VOL = VOL.unstack()
        VOL.replace(0, np.nan, inplace=True)
        VOL = VOL / VOL.sum()
        VOL = np.log(VOL)
        df = DataFrame(VOL.std(), columns=[trade_date]).T
        ...
        df_new.to_sql('tfactorminstr', engine_w, schema='factor', if_exists='append', index=True, chunksize=10000, method=tools.mysql_replace_into)
