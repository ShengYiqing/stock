...
beta5 = (r.ewm(halflife=5, min_periods=60).corr(r_m) * r.ewm(halflife=5, min_periods=60).std()).div(r_m.ewm(halflife=5, min_periods=60).std(), axis=0)
beta5.replace(np.inf, np.nan, inplace=True)
beta5.replace(-np.inf, np.nan, inplace=True)

beta20 = (r.ewm(halflife=20, min_periods=60).corr(r_m) * r.ewm(halflife=20, min_periods=60).std()).div(r_m.ewm(halflife=20, min_periods=60).std(), axis=0)
beta20.replace(np.inf, np.nan, inplace=True)
beta20.replace(-np.inf, np.nan, inplace=True)

rank_beta5 = beta5.rank(axis=1, pct=True)
rank_beta20 = beta20.rank(axis=1, pct=True)
...
df = pd.concat({'r_d_a':r_d_a, 
                ...
                'rank_beta5':rank_beta5,
                'rank_beta20':rank_beta20,
                'rank_mc':rank_mc,
                'rank_pb':rank_pb,
                }, axis=1)
