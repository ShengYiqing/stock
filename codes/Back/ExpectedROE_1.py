...
def generate_factor(start_date, end_date):
    ...
    for trade_date in trade_dates:
        ...
        for i in range(n):
            ...
            tmp.loc[i, :] = df_sql_tmp.set_index(['report_date', 'stock_code']).loc[:, 'roe'].groupby(['report_date', 'stock_code']).mean().unstack().dropna(axis=1, thresh=3).ewm(halflife=20).mean().iloc[-1, :]
    ...
    df_new.to_sql('tfactorexpectedroe', engine, schema='factor', if_exists='append', index=True, chunksize=10000, method=tools.mysql_replace_into)
