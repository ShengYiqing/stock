...
def generate_factor(start_date, end_date):
    ...
    BUY_VOL = BUY_VOL.unstack().replace(0, np.nan) * 100
    SELL_VOL = SELL_VOL.unstack().replace(0, np.nan) * 100
    BUY_AMOUNT = BUY_AMOUNT.unstack().replace(0, np.nan) * 10000
    SELL_AMOUNT = SELL_AMOUNT.unstack().replace(0, np.nan) * 10000
    a = (np.log(SELL_AMOUNT) - np.log(SELL_VOL)) - (np.log(BUY_AMOUNT) - np.log(BUY_VOL))
    df_copy = a.copy()
    n_list = [5, 20, 60, 250]
    for n in n_list:
        ...
        df_new.to_sql('tfactordiffprice%s'%n, engine, schema='factor', if_exists='append', index=True, chunksize=10000, dtype={'STOCK_CODE':VARCHAR(20), 'TRADE_DATE':VARCHAR(8), 'REC_CREATE_TIME':VARCHAR(14)}, method=tools.mysql_replace_into)
