...
def generate_factor(start_date, end_date):
    ...
    for n in n_list:
        try:
            sql = """
            CREATE TABLE `factor`.`tfactorholomean%s` (
            ...
            pass
    sql = """
    select STOCK_CODE, TRADE_DATE, FACTOR_VALUE from tfactorholo
    ...
    for n in n_list:
        df = FACTOR_VALUE.rolling(n, min_periods=int(np.ceil(n*4/5))).mean()
        ...
        df_new.to_sql('tfactorholomean%s'%n, engine, schema='factor', if_exists='append', index=True, chunksize=10000, method=tools.mysql_replace_into)
