...
def generate_factor(start_date, end_date):
    ...
    sql = ' select tsb.industry industry, tm.stock_code, tm.factor_value m '
    for f in f_list:
    ...
    for trade_date in trade_cal:
        ...
        #     result['m'].append(cv['l2-mean'][-1])
        #     result['s'].append(cv['l2-stdv'][-1])
        #     print('---------------')
        #     print('learning_rate=', learning_rate)
        #     print('m=', cv['l2-mean'][-1])
        #     print('s=', cv['l2-stdv'][-1])
        # DataFrame(result).set_index(['num_boost_round', 'learning_rate']).loc[:, 'm'].unstack().to_csv('./e4.csv')
    ...
    df_new.to_sql('tfactorreversal', engine, schema='factor', if_exists='append', index=True, chunksize=5000, method=tools.mysql_replace_into)
