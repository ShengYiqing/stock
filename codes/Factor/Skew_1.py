...
def generate_factor(start_date, end_date):
    ...
    r = CLOSE.diff()
    n = 5
    mu1 = r.ewm(halflife=n, min_periods=5).mean()
    mu2 = (r**2).ewm(halflife=n, min_periods=5).mean()
    mu3 = (r**3).ewm(halflife=n, min_periods=5).mean()

    sigma = r.ewm(halflife=n, min_periods=5).var()

    df = (mu3 - 3 * mu2 * mu1 + 2 * mu1**3) / sigma**1.5
    ...
    df.to_sql('tfactorskew', engine, schema='factor', if_exists='append', index=True, chunksize=10000, method=tools.mysql_replace_into)
