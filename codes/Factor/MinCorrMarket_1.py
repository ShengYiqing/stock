...
def generate_factor(start_date, end_date):
    ...
    try:
        sql = """
        CREATE TABLE `factor`.`tfactormincorrmarket` (
    ...
    for trade_date in trade_cal:
        ...
        r = CLOSE.diff()
        r_m = r.mean(1)
        df = DataFrame(r.corrwith(r_m, axis=0), columns=[trade_date]).T
        ...
        df_new.to_sql('tfactormincorrmarket', engine_w, schema='factor', if_exists='append', index=True, chunksize=10000, method=tools.mysql_replace_into)
