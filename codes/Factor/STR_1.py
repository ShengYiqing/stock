...
def generate_factor(start_date, end_date):
    ...
    df = np.log(df)
    df_copy = df.copy()
    df = df_copy.ewm(halflife=20).std() / df_copy.ewm(halflife=20).mean()
    ...
    df.to_sql('tfactorstr', engine, schema='factor', if_exists='append', index=True, chunksize=10000, dtype={'STOCK_CODE':VARCHAR(20), 'TRADE_DATE':VARCHAR(8), 'REC_CREATE_TIME':VARCHAR(14)}, method=tools.mysql_replace_into)
