...
def generate_factor(start_date, end_date):
    ...
    for trade_date in trade_dates:
        ...
        con_yysr_1 = df_tmp.set_index(['ann_date', 'stock_code']).financial_value.unstack().ewm(halflife=60, min_periods=5).mean().iloc[-1]
        
        end_date_tmp = str(int(trade_date[:4])+1) + '1231'
        start_date_tmp = tools.trade_date_shift(trade_date, 250)
        df_tmp = df_sql.loc[(df_sql.ann_type=='分析师预期')&(df_sql.financial_index=='yysr')&(df_sql.ann_date>=start_date_tmp)&(df_sql.ann_date<=trade_date)&(df_sql.end_date==end_date_tmp)]
        con_yysr_2 = df_tmp.set_index(['ann_date', 'stock_code']).financial_value.unstack().ewm(halflife=60, min_periods=5).mean().iloc[-1]
        
        con_growth = (con_yysr_2 / con_yysr_1 - 1).dropna()
        dic[trade_date] = con_growth
    ...
    df.to_sql('tfactorcongrowth', engine, schema='factor', if_exists='append', index=True, chunksize=5000, method=tools.mysql_replace_into)
