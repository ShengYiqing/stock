...
def generate_factor(start_date, end_date):
    ...
    n_list = [5, 20, 60, 250]
    for n in n_list:
        df = (df_copy*df_copy.shift()).rolling(n, min_periods=int(np.ceil(n*4/5))).mean() - df_copy.rolling(n, min_periods=int(np.ceil(n*4/5))).mean()*df_copy.shift().rolling(n, min_periods=int(np.ceil(n*4/5))).mean()
        ...
        df_new.to_sql('tfactorautocov%s'%n, engine, schema='factor', if_exists='append', index=True, chunksize=10000, dtype={'STOCK_CODE':VARCHAR(20), 'TRADE_DATE':VARCHAR(8), 'REC_CREATE_TIME':VARCHAR(14)}, method=tools.mysql_replace_into)
