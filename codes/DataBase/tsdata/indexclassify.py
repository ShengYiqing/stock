...
for trade_date in trade_cal:
    df = tools.download_tushare(pro=pro, api_name='adj_factor', trade_date=trade_date, fields='ts_code, trade_date, adj_factor')
    ...
    if len(df) > 0:
        ...
        df.to_sql('ttsadjfactor', engine, schema='tsdata', index=False, if_exists='append', method=tools.mysql_replace_into)
