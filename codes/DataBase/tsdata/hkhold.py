...
for trade_date in trade_cal:
    df = tools.download_tushare(pro=pro, api_name='hk_hold', trade_date=trade_date, limit=3800, fields='trade_date,ts_code,vol,ratio,exchange')
    ...
    if len(df) > 0:
        ...
        df.to_sql('ttshkhold', engine, schema='tsdata', index=False, if_exists='append', method=tools.mysql_replace_into)
