...
for trade_date in trade_cal:
    for stock in stock_list:
        df = tools.download_tushare(pro=pro, api_name='stk_surv', ts_code=stock, trade_date=trade_date, fields='ts_code, surv_date, fund_visitors, rece_place, rece_mode, rece_org, org_type, comp_rece')
        if len(df) > 0:
            ...
            df.to_sql('ttsstksurv', engine, schema='tsdata', index=False, if_exists='append', method=tools.mysql_replace_into)
