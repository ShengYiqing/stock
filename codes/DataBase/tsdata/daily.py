...
for trade_date in trade_cal:
    ...
    df = tools.download_tushare(pro=pro, api_name='daily', trade_date=trade_date, fields='ts_code, trade_date, open, high, low, close, pre_close, vol, amount')
    ...
    if len(df) > 0:
        ...
        df.to_sql('ttsdaily', engine, schema='tsdata', index=False, if_exists='append', method=tools.mysql_replace_into)
