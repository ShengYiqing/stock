...
for trade_date in trade_cal:
    df = tools.download_tushare(pro=pro, api_name='moneyflow', trade_date=trade_date, limit=5000)
    ...
    if len(df) > 0:
        ...
        df.to_sql('ttsmoneyflow', engine, schema='tsdata', index=False, if_exists='append', method=tools.mysql_replace_into)
