...
for i in range(iters):
    ...
    df = tools.download_tushare(pro=pro, api_name='stk_rewards', ts_code=stocks, fields='ts_code,ann_date,end_date,name,title,reward,hold_vol')
    if len(df) > 0:
        ...
        df.to_sql('ttsstkrewards', engine, schema='tsdata', index=False, if_exists='append', method=tools.mysql_replace_into)
