...
sql_s_i = """
select stock_code, industry from tsdata.ttsstockbasic
"""
df_s_i = pd.read_sql(sql_s_i, engine).set_index('stock_code').loc[:, 'industry']
df.loc[:, 'ind'] = [gc.INDUSTRY_DIC_INV[df_s_i.loc[i]] if i in df_s_i.index else 'iuk' for i in df.loc[:, 'stock_code']]
