...
sql_s_i = """
select stock_code, l1_name ind_1, l2_name ind_2 from indsw.tindsw
where (not isnull(l1_name)) and (not isnull(l2_name))
"""
df_s_i = pd.read_sql(sql_s_i, engine).set_index('stock_code').loc[:, ['ind_1', 'ind_2']]
...
df = df.set_index(['trade_date', 'stock_code', 'ind_1', 'ind_2'])
...
df.loc[:, 'score'] = (
    df.loc[:, factor_ks].mean(1).groupby(['trade_date']).apply(lambda x:x.rank()/x.notna().sum()) + 
    df.loc[:, factor_ks].mean(1).groupby(['trade_date', 'ind_1']).apply(lambda x:x.rank()/x.notna().sum()) + 
    df.loc[:, factor_ks].mean(1).groupby(['trade_date', 'ind_2']).apply(lambda x:x.rank()/x.notna().sum())
    ) / 3
...
df.loc[:, 'top'] = (df.loc[:, 'top_1'] | df.loc[:, 'top_2']).astype(int)
df = df.loc[:, ['score', 'top'] + factor_ks]
df.loc[:, 'REC_CREATE_TIME'] = datetime.datetime.today().strftime('%Y%m%d%H%M%S')
df = df.reset_index(['ind_1', 'ind_2'], drop=True)
